...
export class OrdersService {
  ...
  private async adjustStockTx(
    ...
    params: { productId: number; variantId?: number | null; delta: number }
  ) {
    ...
    if (variantId) {
      // Pre-read to validate existence, product match, and handle NULL (untracked)
      const current = await tx.variant.findUnique({
        where: { id: variantId },
        select: { stock: true, productId: true },
      });
      if (!current || current.productId !== productId) {
        throw new BadRequestException("Variant not found to adjust");
      }
      if (current.stock == null) return; // treat NULL as untracked → no-op

      // Atomic guard for decrements; no guard needed for increments
      const where = isDecrement
        ? { id: variantId, productId, stock: { gte: abs } }
      ...
      if (res.count !== 1) {
        // If decrement, it's usually because stock < abs; otherwise not found
        throw new BadRequestException(
          isDecrement
            ? "Insufficient variant stock"
            : "Variant not found to increment"
        );
      }
    } else {
      ...
      if (!current) {
        throw new BadRequestException("Product not found to adjust");
      }
      if (current.stock == null) return; // untracked → no-op

      const where = isDecrement
        ? { id: productId, stock: { gte: abs } }
        : { id: productId };
      ...
      if (res.count !== 1) {
        throw new BadRequestException(
          isDecrement
            ...
            : "Product not found to increment"
        );
      }
    }
  }
  ...
  async placeOrder(dto: CreateOrderDto, userId: number) {
    ...
    // Create order
    // === REPLACE the "Create order" section in placeOrder() with this transaction ===
    const order = await this.prisma.$transaction(async (tx) => {
      const created = await tx.order.create({
        data: {
          ...
          payment: {
            create: { method: paymentMethod ?? "COD", status: "PENDING" },
          },
        },
        include: { items: true, payment: true },
      });

      // Decrement stock for each item (atomic with order creation)
    ...
    // Mark coupon usage (outside of tx is fine; failure shouldn’t rollback order)
    if (couponId) {
      try {
  ...
  async listUserOrders(userId: number) {
    ...
    for (const catId of uniqueCatIds) {
      const cat = await (this.prisma as any).category.findUnique({
        where: { id: catId },
      });
      if (cat?.parentId) {
    ...
    const relatedRaw = uniqueSiblingIds.length
      ? await this.prisma.product.findMany({
          where: {
            status: true,
            id: {
              notIn: purchasedProductIds.length
                ? Array.from(new Set(purchasedProductIds))
                : undefined,
            },
            OR: [
      ...
      items: (ord.items || []).map((it: any) => {
        ...
        return {
          ...
          product: {
            ...
            color: useVariant
              ? it.variant?.color?.label
              : it.product?.color?.label,
            size: useVariant
              ? it.variant?.size?.title
              : it.product?.size?.title,
            imageUrl: formatImageUrl(mainImage),
            imageAlt,
          },
          price:
            it.price ?? it.sellingPrice ?? it.product?.sellingPrice ?? null,
  ...
  async getOrderDetails(orderId: number) {
    ...
    if (order.coupon && typeof (order as any).couponDiscount === "number") {
      couponDiscount = Math.max(
        0,
        Math.min(totalSellingPrice, Number((order as any).couponDiscount) || 0)
      );
    } else {
      // Legacy fallback: infer from totals
      couponDiscount =
        totalSellingPrice + platformFee + shippingFee - storedFinal;
      couponDiscount = Math.max(
        0,
    ...
    const safeAddress = (order as any).addressSnapshot
  ...
  async buyNow(dto: BuyNowDto, userId: number) {
    ...
    const safeDiscount = Math.max(
      0,
      Math.min(subtotal, Number(couponDiscount) || 0)
    );
    ...
    if (couponId != null) {
      ...
      if (!coupon) {
        throw new BadRequestException(
          `Invalid or inactive coupon ID: ${couponId}`
        );
      }
    ...
    // Create order
    // === REPLACE the "Create order" + "Decrement stock" in buyNow() with this transaction ===
    const order = await this.prisma.$transaction(async (tx) => {
      const created = await tx.order.create({
        data: {
          user: { connect: { id: userId } },
  ...
  async requestCancelItem(
    ...
    if (ORDER_FINAL_STATES.includes(item.order.status)) {
      throw new BadRequestException(
        `Order is ${item.order.status}; item cannot be changed`
      );
    }
  ...
  /** Admin approves or cancels an item (APPROVED / CANCELLED) */
  async updateOrderItemStatus(itemId: number, body: UpdateOrderItemDto) {
    // Load item with fields needed for stock adjustments + sibling statuses
    const item = await this.prisma.orderItem.findUnique({
      where: { id: itemId },
      select: {
        id: true,
    ...
    if (next !== "APPROVED" && next !== "CANCELLED") {
      throw new BadRequestException("status must be APPROVED or CANCELLED");
    }
    const nextEnum =
      next === "APPROVED"
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      // 2) If we *moved into* CANCELLED (and it wasn't already CANCELLED), credit stock back
      if (
        ...
        await this.adjustStockTx(tx, {
          productId: item.productId,
          variantId: item.variantId ?? null,
          delta: +Number(item.quantity || 1), // increment back
        });
      }

      // 3) If now all items are CANCELLED -> mark order CANCELLED
      const siblings = await tx.orderItem.findMany({
        where: { orderId: item.orderId },
        select: { id: true, status: true },
      });

      const allCancelled = siblings.every((s) =>
        s.id === itemId
          ? nextEnum === OrderItemStatus.CANCELLED
          : s.status === OrderItemStatus.CANCELLED
      );
