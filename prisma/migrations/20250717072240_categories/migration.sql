/*
  ...
  - You are about to drop the `category_features` table. If the table is not empty, all the data it contains will be lost.
  - You are about to drop the `category_filters` table. If the table is not empty, all the data it contains will be lost.
...
-- AlterTable
ALTER TABLE `categories` ADD COLUMN `featureTypeId` INTEGER NULL,
    ADD COLUMN `filterTypeId` INTEGER NULL;

-- DropTable
DROP TABLE `category_features`;

-- DropTable
DROP TABLE `category_filters`;
...
-- AddForeignKey
ALTER TABLE `categories` ADD CONSTRAINT `categories_featureTypeId_fkey` FOREIGN KEY (`featureTypeId`) REFERENCES `feature_types`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `categories` ADD CONSTRAINT `categories_filterTypeId_fkey` FOREIGN KEY (`filterTypeId`) REFERENCES `filter_types`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
